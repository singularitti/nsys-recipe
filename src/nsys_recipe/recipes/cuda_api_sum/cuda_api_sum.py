...
class CudaApiSum(recipe.Recipe):
    @staticmethod
    def _mapper_func(report_path, parsed_args):
        service = DataService(report_path, parsed_args)

        service.queue_table("StringIds")
        service.queue_table("CUPTI_ACTIVITY_KIND_RUNTIME", ["nameId", "start", "end"])

        df_dict = service.read_queued_tables()
        if df_dict is None:
            return None

        runtime_df = df_dict["CUPTI_ACTIVITY_KIND_RUNTIME"]
        err_msg = service.filter_and_adjust_time(runtime_df)
        if err_msg is not None:
            logger.error(f"{report_path}: {err_msg}")
            return None

        runtime_df = data_utils.replace_id_with_value(
            runtime_df, df_dict["StringIds"], "nameId", "name"
        )

        if runtime_df.empty:
            logger.info(
                f"{report_path}: Report was successfully processed, but no data found."
            )
            return None

        runtime_df["duration"] = runtime_df["end"] - runtime_df["start"]
        runtime_df = runtime_df[runtime_df["duration"] > 0]
        kernel_gdf = runtime_df[["name", "duration"]].groupby("name")

        filename = Path(report_path).stem
        stats_df = summary.describe_column(kernel_gdf["duration"])
