...
class UcxGpuTimeUtilMap(recipe.Recipe):
    @staticmethod
    def _mapper_func(report_path, max_duration, session_offset, bin_size, parsed_args):
        ...
        service.queue_custom_table(CompositeTable.UCX)
        ...
        ucx_grouped = ucx_df.groupby("pid")

        unique_pids = set(kernel_grouped.groups.keys()) | set(ucx_grouped.groups.keys())
        results = []
        ...
        for pid in unique_pids:
            ...
            bin_pcts = {key: zero_bins for key in ("Kernel", "UCX", "Overlap")}

            groups_info = [("Kernel", kernel_grouped), ("UCX", ucx_grouped)]
            for name, grouped in groups_info:
                if pid in grouped.groups:
                    group_dfs[name] = overlap.consolidate_ranges(grouped.get_group(pid))

            # Calculate the overlap only if 'Kernel' and 'UCX' both exist for
            # the given PID.
            if "Kernel" in group_dfs and "UCX" in group_dfs:
                overlap_range_df = overlap.calculate_overlapping_ranges(
                    group_dfs["Kernel"], group_dfs["UCX"]
                )
