...
class CudaGpuMemTimeSum(recipe.Recipe):
    @staticmethod
    def _mapper_func(report_path, parsed_args):
        ...
        service.queue_table(
            "CUPTI_ACTIVITY_KIND_MEMCPY", ["start", "end", "copyKind", "deviceId"]
        )
        service.queue_table("CUPTI_ACTIVITY_KIND_MEMSET", ["start", "end", "deviceId"])
        ...
        err_msg = service.filter_and_adjust_time(memory_df)
        ...
        memory_df["duration"] = memory_df["end"] - memory_df["start"]
        memory_df = memory_df[memory_df["duration"] > 0]

        stats_df = summary.describe_column(memory_df.groupby("name")["duration"])
        stats_df.index.name = "Name"

        stats_by_device_df = summary.describe_column(
            memory_df.groupby(["name", "deviceId"])["duration"]
        )
