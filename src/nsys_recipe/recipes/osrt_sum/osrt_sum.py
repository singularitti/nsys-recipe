...
class OsrtSum(recipe.Recipe):
    ...
    def _mapper_func(report_path, parsed_args):
        ...
        osrt_df = df_dict["OSRT_API"]
        err_msg = service.filter_and_adjust_time(osrt_df)
        if err_msg is not None:
            logger.error(f"{report_path}: {err_msg}")
            return None

        osrt_df = data_utils.replace_id_with_value(
            osrt_df, df_dict["StringIds"], "nameId", "name"
        )
        ...
        osrt_df["duration"] = osrt_df["end"] - osrt_df["start"]
        osrt_df = osrt_df[osrt_df["duration"] > 0]
        kernel_gdf = osrt_df[["name", "duration"]].groupby("name")
