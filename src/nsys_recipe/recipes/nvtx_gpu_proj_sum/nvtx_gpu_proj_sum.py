...
from nsys_recipe.lib import cuda, data_utils, helpers, nvtx, recipe, summary
...
class NvtxGpuProjSum(recipe.Recipe):
    @staticmethod
    def _mapper_func(report_path, parsed_args):
        ...
        # Calculate the statistics for the projected ranges.
        proj_nvtx_df["duration"] = proj_nvtx_df["end"] - proj_nvtx_df["start"]
        proj_nvtx_df = proj_nvtx_df[proj_nvtx_df["duration"] > 0]
        range_gdf = proj_nvtx_df[["text", "duration"]].groupby("text")

        stats_df = summary.describe_column(range_gdf["duration"])

        # Calculate the total duration for the original ranges.
        nvtx_df["duration"] = nvtx_df["end"] - nvtx_df["start"]
        nvtx_df = nvtx_df[nvtx_df["duration"] > 0]

        duration_sum = nvtx_df.groupby("text")["duration"].sum()
        stats_df["Sum of NVTX Ranges on CPU"] = duration_sum
    ...
    def reducer_func(self, mapper_res):
        ...
        all_stats_df = summary.aggregate_stats_df(
            stats_df, column_agg_dict={"Sum of NVTX Ranges on CPU": "sum"}
        )
    ...
    def save_notebook(self):
        ...
        self.create_notebook("topN.ipynb")
